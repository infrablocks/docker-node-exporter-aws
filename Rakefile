...
namespace :prerelease do
  ...
  task :publish, %i[pr_number run_number run_attempt] do |_, args|
    ...
    image =
      begin
        Docker::Image.get('node-exporter-aws:latest')
      rescue Docker::Error::NotFoundError
        raise RakeDocker::ImageNotFound, 'node-exporter-aws image not built'
      end

    image.tag(repo: 'infrablocks/node-exporter-aws', tag: pr_tag,
              force: true)
    ...
    # (the bare local build tag, e.g. 'node-exporter-aws:latest'), so the
    # push targets docker.io/library/<name> where the PR tag does not
    # exist. The tag: option only sets the tag, never the repository.
    image.push(nil,
               repo_tag: "infrablocks/node-exporter-aws:#{pr_tag}") do |chunk|
      RakeDocker::Output.print chunk
    end

    puts "Published infrablocks/node-exporter-aws:#{pr_tag}"
  end
end
